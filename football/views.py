...
def predict_match(request, match_id):
    ...
    # Reset daily selections if needed
    UserSportPrediction.reset_daily_selections()
    
    # Check if any sport is selected for today
    selected_sport = UserSportPrediction.objects.filter(
        user=request.user,
        is_selected_for_today=True
    ).first()
    
    # If a sport is selected and it's not the current sport, block access
    if selected_sport and selected_sport.sport != match.sport:
        # Check if the selected sport's last prediction is older than 24 hours
        if timezone.now() - selected_sport.last_prediction > timedelta(days=1):
            selected_sport.is_selected_for_today = False
            selected_sport.save()
        else:
            messages.error(
                request, 
                f'شما امروز برای {selected_sport.sport.get_name_display()} پیش‌بینی کرده‌اید. '
                f'می‌توانید فردا برای {match.sport.get_name_display()} پیش‌بینی کنید.'
            )
            return redirect('football:home')
    ...
    needs_payment = not (sport_prediction and sport_prediction.has_paid)
    if needs_payment:
        if profile.balance < 5:
            messages.error(request, 'برای پیش‌بینی در این ورزش باید حداقل ۵ دلار موجودی داشته باشید.')
            return redirect('football:home')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                # Create or update sport prediction after successful prediction
                ...
                # Deduct $5 only if this is the first prediction for this sport type
                if needs_payment:
                    profile.balance -= 5
                    profile.save()
                    sport_prediction.has_paid = True
                    sport_prediction.save()
                    messages.success(request, '۵ دلار از حساب شما کسر شد و پیش‌بینی شما با موفقیت ثبت شد.')
                else:
...
def leaderboard(request):
    ...
    # Get top 5 users by points from their profiles
    top_users = User.objects.select_related('user_profile').order_by('-user_profile__points')[:5]

    # Get current user's rank
    user_rank = User.objects.filter(
        user_profile__points__gt=request.user.user_profile.points
    ).count() + 1

    # Get user statistics for each top user
    for user in top_users:
        user.predictions_count = Prediction.objects.filter(user=user).count()
        user.correct_predictions = Prediction.objects.filter(user=user, is_correct=True).count()
        user.accuracy = round((user.correct_predictions / user.predictions_count * 100) if user.predictions_count > 0 else 0)
...
def introduction(request):
    return render(request, 'football/introduction.html')

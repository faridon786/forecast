...
class Prediction(models.Model):
    ...
    def check_prediction(self):
        ...
        if pred_home == actual_home and pred_away == actual_away:
            ...
            points_to_award = 10
        # Equal prediction (same difference): 10 points
        elif (pred_home - pred_away) == (actual_home - actual_away):
            self.is_correct = True
            points_to_award = 10
        # Reversed prediction: 7 points (mark as correct for display)
        elif pred_home == actual_away and pred_away == actual_home:
            self.is_correct = True  # Changed to True so it shows as green
            points_to_award = 7
        # Wrong prediction: 2 points (participation bonus)
        else:
            self.is_correct = False
            points_to_award = 2
        
        # Save prediction status and award points
        self.save()
